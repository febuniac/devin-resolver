...
export default function Security() {
  ...
  const getName = (f: Finding) => (f.title || f.rule || '').replace(/^\[SECURITY\]\s*/i, '');
  const getRepo = (f: Finding) => f.repo_full_name || f.repo || '';
  const getScore = (f: Finding) => f.ai_confidence || f.confidence || 0;
    ...
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--dim)', marginTop: 2 }}>Security issues across connected repositories</div>
        </div>
      </div>
          ...
          <div key={f.id} style={{ display: 'grid', gridTemplateColumns: '1fr 90px 80px 90px 80px', padding: '12px 16px', borderBottom: '1px solid var(--rule)', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--ink)', lineHeight: 1.35, display: 'flex', alignItems: 'center', gap: 6 }}>
                {f.severity === 'critical' && <AlertTriangle size={14} style={{ color: '#e53e3e', flexShrink: 0 }} />}
                {getName(f)}
              </div>
              <div className="font-mono" style={{ fontSize: 10, color: 'var(--dim)', marginTop: 3 }}>{getRepo(f)}</div>
            </div>
            <div>
            ...
            <div><span className="font-mono" style={{ fontSize: 12, fontWeight: 700, color: getScore(f) >= 75 ? 'var(--green)' : '#d97706' }}>{getScore(f)}</span></div>
            <div>
            ...
            <div>
              {(f.status === 'triaged' || f.status === 'open') ? (
                ...
                <a href={f.pr_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, fontWeight: 600, color: 'var(--purple)', textDecoration: 'none' }}>View PR</a>
